...
class DialogueManager:
    def __init__(self, socketio):
        ...
        self.subject_prompt_variants = [
            ...
            "Отлично! У меня есть уроки по: {subjects}. Что выбираете?",
            "Прекрасно! Давайте выберем тему. Доступно: {subjects}.",
            "Рад вашему интересу к обучению! Выбирайте предмет: {subjects}."
        ]
        ...
        self.local_patterns = {
            ...
            "начать": ["Отлично! Давайте начнем обучение.", "Прекрасно! Готов начать урок."],
            "поехали": ["Отлично! Поехали!", "Замечательно! Начинаем наш урок."],
            "дальше": ["Переходим к следующей части.", "Продолжаем урок..."],
            "еще": ["Конечно, продолжаем!", "Еще больше интересной информации!"]
        }
    ...
    def _detect_lesson_category(self, topic: str) -> str:
        """Определяет категорию урока по теме"""
        topic_lower = topic.lower()
        
        programming_keywords = ['swift', 'python', 'java', 'javascript', 'программирование', 
                               'код', 'алгоритм', 'разработка', 'ios', 'android', 'веб',
                               'сайт', 'приложение', 'игра', 'мобильн', 'desktop', 'сервер',
                               'база данных', 'sql', 'html', 'css', 'фреймворк', 'библиотека',
                               'git', 'github', 'debug', 'компиляция', 'интерфейс', 'api']
        
        science_keywords = ['физика', 'химия', 'биология', 'математика', 'алгебра', 'геометрия',
                           'астрономия', 'генетика', 'квант', 'молекула', 'атом', 'реакция',
                           'формула', 'теорема', 'уравнение', 'лабораторная', 'эксперимент']
        ...
        language_keywords = ['английский', 'русский', 'немецкий', 'французский', 'испанский',
                            ...
                            'иероглиф', 'алфавит', 'произношение', 'акцент', 'диалект']
    ...
    def generate_lesson_on_demand(self, topic: str) -> Optional[dict]:
        ...
        try:
            ...
            category = self._detect_lesson_category(topic)
            
            system_prompt = f"""Ты - эксперт-преподаватель и создатель образовательных материалов. 
Создай подробный, структурированный и engaging урок по теме: {topic}

Требования к уроку:
1. Будь информативным и точным
2. Раздели на логические абзацы (по 2-3 предложения, разделяй пустыми строками)
3. Адаптируй для учеников разного уровня
4. Используй только русский язык
5. Добавь практические примеры и реальные кейсы
6. Сделай урок интересным и вовлекающим
7. Объясни сложные понятия простыми словами
8. Включи ключевые выводы и итоги
...
- Ключевые выводы и рекомендации"""
            ...
            lesson_content = self.llm._query_llm_api(
                prompt=f"Создай comprehensive образовательный урок на тему: {topic}. Урок должен быть полным, структурированным и полезным для студентов.",
                context="",
                subject=category,
                system_prompt=system_prompt,
                max_tokens=3000
            ...
            if not lesson_content:
                print("LLM не вернула содержание урока")
                return None
            
            safe_topic = re.sub(r'[^\w\s]', '', topic).replace(' ', '_').lower()[:50]
            lesson_id = f"generated_{safe_topic}_{int(time.time())}"
            ...
            self.lessons[category].append(lesson_data)
    ...
    def _is_lesson_generation_request(self, text: str) -> Optional[str]:
                        ...
                        if generated_lesson:
                            ...
                            if self.socketio and self.room_id:
                                self.socketio.emit('llm_response_ready', {
                                    'room_id': self.room_id,
                                    'question': text,
                                    'answer': f"❌ К сожалению, не удалось создать урок по теме '{topic}'. Попробуйте другую тему или выберите из существующих предметов."
                                })
                    except Exception as e:
                        print(f"Ошибка асинхронной генерации урока: {e}")
                
                thread = threading.Thread(target=generate_lesson_async)
                thread.daemon = True
                thread.start()
    ...
    def handle_question_during_lesson(self, question: str) -> str:
        ...
        immediate_response = "Интересный вопрос! Сейчас подумаю над подробным ответом..."
