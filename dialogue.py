...
class DialogueManager:
    def __init__(self, socketio):
        ...
        self.local_patterns = {
            ...
            "математика": ["Математика - отличный выбор!", "Отлично! Математика."],
            "обществознание": ["Обществознание - интересный предмет!", "Хорошо! Обществознание."],
            "русский": ["Русский язык - важно знать!", "Отлично! Русский язык."],
            "физика": ["Физика - увлекательная наука!", "Хорошо! Физика."],
            "химия": ["Химия - это интересно!", "Отлично! Химия."],
            "биология": ["Биология - изучаем живую природу!", "Хорошо! Биология."],
            "история": ["История - познаем прошлое!", "Отлично! История."],
            "английский": ["Английский язык - полезно знать!", "Хорошо! Английский."],
            "информатика": ["Информатика - современный предмет!", "Отлично! Информатика."],
            "готов": "LESSON_START_SIGNAL",  # ИЗМЕНЕНИЕ: сразу возвращаем сигнал
            "да": "LESSON_START_SIGNAL",     # ИЗМЕНЕНИЕ: "да" тоже запускает урок
        }
    ...
    def process_input(self, text: str) -> str:
        ...
        for pattern, responses in self.local_patterns.items():
            if pattern in text_lower:
                # Если это сигнал начала урока, возвращаем сразу
                ...
                # Для остальных шаблонов возвращаем случайный ответ
    ...
    def _handle_greeting(self, text: str) -> Optional[str]:
        ...
        subject_words = ["математика", "обществознание", "русский", "физика", "химия", 
                        "биология", "история", "английский", "informтика", "урок"]
    ...
    def handle_question_during_lesson(self, question: str) -> str:
        ...
        for pattern, responses in self.local_patterns.items():
            if pattern in question_lower:
                ...
                if responses == "LESSON_START_SIGNAL":
                    continue  # Пропускаем команды начала урока
                if isinstance(responses, list):
                    return random.choice(responses)
                return responses
