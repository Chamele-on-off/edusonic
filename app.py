...
# ИСПРАВЛЕНИЕ: Создаем DialogueManager в отдельных потоках
...
def initialize_dialogue_manager(room_id):
    """Инициализирует DialogueManager для комнаты в отдельном потоке"""
    try:
...
def handle_join_room(data):
    ...
    # Инициализируем DialogueManager асинхронно
    if room_id not in room_dialogue or room_dialogue[room_id] is None:
        threading.Thread(target=initialize_dialogue_manager, args=(room_id,), daemon=True).start()
...
def handle_activate_ai_teacher(data):
    ...
    threading.Thread(target=init_and_greet, daemon=True).start()
