...
def trigger_auto_continue(room_id):
    ...
    try:
        ...
        # Получаем следующий абзац через диалог менеджер
        next_paragraph = dialogue._get_next_paragraph()
        ...
        if next_paragraph:
            ...
            # Озвучиваем следующий абзац через существующую систему
            speak_text(room_id, next_paragraph, voice_type='female', is_teacher=True)
            
            # ЗАПУСКАЕМ ТАЙМЕР ДЛЯ СЛЕДУЮЩЕГО АВТОПРОДОЛЖЕНИЯ
            if hasattr(dialogue, 'lesson_manager'):
                dialogue.lesson_manager.set_auto_continue_callback(None, room_id)
                dialogue.lesson_manager.start_auto_continue_timer()
            ...
            # Урок завершен
            print("🏁 Урок завершен по таймеру")
            practice_msg = "Урок завершен. Переходим к практике."
            ...
            # Запускаем практику
            dialogue._start_practice_session()
