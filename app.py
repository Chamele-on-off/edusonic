...
# =============================================================================
# ИНФОГРАФИКА - НОВАЯ СИСТЕМА ВИЗУАЛИЗАЦИИ
# =============================================================================
...
class InfographicGenerator:
    ...
    def generate_infographic(self, topic: str, context: str = "") -> dict:
        ...
        try:
            ...
            # Генерируем структуру инфографики
    ...
    def _generate_structure(self, topic: str, context: str) -> Optional[dict]:
        """Генерирует структуру инфографики"""
        # Упрощенная структура на основе темы
        words = topic.split()[:4]  # Берем первые 4 слова как ключевые элементы
        
        elements = []
        for i, word in enumerate(words):
            elements.append({
                "type": "key_point",
                "content": word.capitalize(),
                "importance": min(i + 1, 5)
            })
        
        # Добавляем дополнительные элементы на основе контекста
        if context:
        ...
        return {
            ...
            "elements": elements[:6],  # Максимум 6 элементов
            "layout": random.choice(["vertical", "horizontal", "grid"])
        }
    ...
    def _create_vertical_layout(self, elements: list, style: dict) -> list:
        ...
        for i, element in enumerate(elements):
            ...
            if elem_type == "header":
                svg_elements.extend([
                    f'<rect x="50" y="{y_position}" width="300" height="40" rx="{style["border_radius"]}" fill="{color}" opacity="0.8"/>',
    ...
    def _create_horizontal_layout(self, elements: list, style: dict) -> list:
        ...
        for i, element in enumerate(elements[:4]):  # Максимум 4 элемента
            ...
            svg_elements.extend([
                f'<rect x="{x_positions[i]-30}" y="{y_position-40}" width="60" height="60" rx="{style["border_radius"]}" fill="{color}" opacity="0.7"/>',
                f'<text x="{x_positions[i]}" y="{y_position}" text-anchor="middle" class="text key-point" fill="white">{content}</text>'
            ])
            
        return svg_elements
    
    def _create_grid_layout(self, elements: list, style: dict) -> list:
    ...
    def _create_fallback_svg(self, title: str, style: dict) -> str:
        ...
        <svg width="400" height="300" xmlns="http://www.w3.org/2000/svg" style="background:{style['background']}">
            ...
            <rect x="50" y="50" width="300" height="200" rx="{style['border_radius']}" fill="{style['colors'][0]}" opacity="0.1"/>
...
# Глобальный экземпляр генератора инфографики
infographic_generator = InfographicGenerator()

def generate_visualization(room_id: str, topic: str, context: str = ""):
    """Генерация инфографики вместо Mermaid"""
    debug_log(f"🎨 Генерация инфографики для комнаты {room_id}: {topic[:100]}...")
    
    try:
        # Генерируем инфографику
        result = infographic_generator.generate_infographic(topic, context)
        
        if result and result.get("success"):
            # Добавляем в очередь визуализаций
            ...
            socketio.emit('infographic_generated', {
                'room_id': room_id,
                'topic': topic,
                'svg_code': result['svg_code'],
                'style': result['style'],
            ...
            debug_log(f"✅ Инфографика сгенерирована и отправлена в комнату {room_id}")
            return True
...
def add_visualization_to_queue(room_id, topic, context):
    """Добавляет инфографику в очередь для комнаты"""
    return generate_visualization(room_id, topic, context)
...
@socketio.on('infographic_generated')
def handle_infographic_generated(data):
    """Обработчик готовых инфографик"""
    room_id = data['room_id']
    
    debug_log(f"Получена готовая инфографика для комнаты {room_id}: {data['topic'][:100]}...")
    
    emit('infographic_generated', {
        'room_id': room_id,
        'topic': data['topic'],
...
def handle_generate_visualization(data):
    """Обработчик запроса генерации инфографики - НЕМЕДЛЕННАЯ ГЕНЕРАЦИЯ"""
    room_id = data['room_id']
    ...
    debug_log(f"WebSocket генерация инфографики для комнаты {room_id}: {topic[:100]}...")
    
    success = generate_visualization(room_id, topic, context)
        ...
        if room_id in room_visualization_queue and room_visualization_queue[room_id]:
            ...
            if socketio and room_id:
                socketio.emit('infographic_generated', {
                    ...
                    'style': visualization.get('style', 'modern'),
...
def generate_diagram():
    """Генерация инфографики через API"""
    try:
        data = request.json
        topic = data.get('topic', '')
        context = data.get('context', '')
        ...
        return jsonify({
            "success": success,
            "message": "Инфографика сгенерирована" if success else "Ошибка генерации",
            "topic": topic,
            "type": "infographic"
        })
...
def force_visualization():
    """Принудительная генерация инфографики для тестирования"""
    try:
        data = request.json
        room_id = data.get('room_id', 'default')
        ...
        debug_log(f"Принудительная генерация инфографики для комнаты {room_id}")
        
        success = generate_visualization(room_id, topic, context)
        
        return jsonify({
            "success": success,
            "message": f"Инфографика {'добавлена' if success else 'не сгенерирована'} для комнаты {room_id}",
            "topic": topic
        })
