...
def update_student_lesson_progress(student_id, subject, lesson_id, completed=True):
    ...
    try:
        ...
        if completed and lesson_id not in subject_progress["completed_lessons"]:
            ...
            subject_progress["last_accessed"] = time.time()
...
def upload_keys_file():
    ...
    try:
        ...
        if file and file.filename.endswith('.txt'):
            ...
            # Импортируем ключи
            key_manager = get_key_manager()
            imported_count = key_manager.import_keys_from_file(content)
...
def set_key_limit_route():
    ...
    try:
        ...
        if not key_name or not limit_type:
            return jsonify({"success": False, "error": "Key name and limit type are required"})
        
        if limit_type not in ['standard', 'extended']:
            return jsonify({"success": False, "error": "Invalid limit type. Use 'standard' or 'extended'"})
        ...
        if success:
            return jsonify({
                "success": True,
                "message": f"Лимит ключа {key_name} установлен на '{limit_type}'",
                "key_name": key_name,
...
@teacher_required
def set_reset_time_route():
    """Установка времени сброса счетчиков"""
    try:
        data = request.json
        reset_time = data.get('reset_time')
        
        if not reset_time:
            return jsonify({"success": False, "error": "Reset time is required"})
        ...
        if success:
            ...
            return jsonify({"success": False, "error": "Failed to set reset time"})
...
@teacher_required
def set_model_route():
    """Быстрая смена модели OpenRouter"""
    try:
        data = request.json
        model = data.get('model')
        ...
        if success:
            ...
            return jsonify({"success": False, "error": "Failed to set model"})
...
def force_reset_keys():
    ...
    try:
        key_manager = get_key_manager()
        key_manager._check_daily_reset(force=True)
        
        return jsonify({
            "success": True,
            "message": "Счетчики ключей сброшены"
        })
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})

@app.route('/api/keys/advanced_stats')
@teacher_required
def get_advanced_keys_stats():
    ...
    try:
        ...
        stats['time_to_next_reset'] = key_manager._get_next_reset_time()
